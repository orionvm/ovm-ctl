...
def runconsole(vmname, istty, api):
	...
	errors: 'VM does not exist: Return exit code 3
	        ...
	        .ssh exits with non-zero code: Return exit code 4+(ssh exit code)
	        .    ssh exit codes greater than 60 return 64 instead.'
	"""
	...
	if ret:
		if istty:
			print 'Remote shell failed with exit code %d'
		if ret > 60:
			ret = 60
		return 4+ret
