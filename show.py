	...
	for disk in disks:
		if disk['name'] == diskname:
			...
			if istty:
				...
				print '\t'.join([disk['name'], disk['image'], disk['size']] + (disk['locked'] and ['locked'] or []))
			break
	else:
		if istty:
...
def showvms(istty, api):
	...
	if istty:
		print "Name\tMemory\tDisks\tIPs\tState"
	for vm in vms:
		if istty:
			if vm['ram'] >= 2048:
				...
				vm['ram'] = "%dM" % vm['ram']
		print "\t".join([vm['hostname'], str(vm['ram']), str(len(vm['disks'])), str(len(vm['ips'])), vmstates.get(vm['state'], 'Erroring')])
	return 0
...
def showvm(vmhost, istty, api):
	...
	if istty:
		if vm['ram'] >= 2048:
			...
			ram = "%dM" % vm['ram']
		print '%s has size %s and is currently %s' % (vm['hostname'], ram, vmstates.get(vm['state'], "Erroring"))
	else:
		print '%s\t%d\t%s' % (vm['hostname'], vm['ram'], vmstates.get(vm['state'], "Erroring"))
